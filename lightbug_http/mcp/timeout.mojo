"""MCP Timeout management system.

This module implements the timeout functionality as specified in the MCP specification:
- Request timeouts with configurable per-request settings
...
---mojo
var timeout_manager = TimeoutManager()  # Uses default 30 second timeout
---

### 2. Custom Global Configuration
---mojo
var config = TimeoutConfig(
    default_timeout_ms=60000,    # 1 minute default
    maximum_timeout_ms=600000,   # 10 minute maximum
    progress_reset_timeout_ms=30000,  # 30 seconds extension on progress
    enable_progress_reset=True
)
var timeout_manager = TimeoutManager(config)
---

### 3. Per-Request Custom Timeouts
---mojo
# Add request with custom 45 second timeout
timeout_manager.add_request(request, custom_timeout_ms=45000)

# The timeout will be capped by maximum_timeout_ms if it exceeds the limit
---

### 4. For Testing Long Operations (like slow_operation tool):
---mojo
# To test timeouts, configure shorter timeouts than operation duration:
var test_config = TimeoutConfig(
    default_timeout_ms=10000,    # 10 seconds (shorter than 33s operation)
    maximum_timeout_ms=20000,    # 20 seconds maximum
    enable_progress_reset=False  # Don't extend timeout on progress
)
var timeout_manager = TimeoutManager(test_config)
---

### 5. Integration with MCP Server:
The timeout manager is integrated into the MCP server and will automatically:
- Track all incoming requests
- Send cancellation notifications when timeouts expire
- Clean up expired requests
- Provide timeout error responses

### Constants Available:
- DEFAULT_REQUEST_TIMEOUT: 30000ms (30 seconds)
- MAXIMUM_REQUEST_TIMEOUT: 300000ms (5 minutes)
- PROGRESS_RESET_TIMEOUT: 60000ms (1 minute)
"""
